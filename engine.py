...
ENGINE_VERSION = "2.4.4-girls-skip"
MAX_RETRIES = 3
...
def _normalize_order3(value):
    """Normalize WINTICKET mark glyph variants before final comparison."""
    if value is None:
        return ""
    s = str(value).strip()
    trans = {
...
def is_girls_race(body_text):
    """
    WINTICKET本文からガールズ競輪を安全側で判定する。
    ガールズは通常ライン戦の対象外なので、指定ライン検索から除外する。
    """
    s = str(body_text or "")
    compact = re.sub(r"\s+", "", s)

    # 明示表記
    if re.search(r"ガールズ(?:ケイリン|競輪)?", compact, re.I):
        return True

    # WINTICKETで使われるL級表記（半角/全角Lを許容）
    if re.search(r"[LＬ]級", compact, re.I):
        return True
    ...
    try:
        ...
        for ri, (result_url, sid, day_no, race_no) in enumerate(candidates, 1):
            ...
            try:
                ...
                # ガールズ競輪は通常ライン戦ではないため、
                # 3.2.2 / 2.3.2 / 2.2.3 検索の対象外として早期スキップする。
                if is_girls_race(pred_body):
                    _race_diag(venue, race_no, grade=grade, result="SKIP_GIRLS")
                    _log(f"SKIP_GIRLS venue={venue} race={race_no}R")
                    continue
                ...
                order3 = _normalize_order3(order3)
                ...
                _star2_diag(venue, race_no, "FINAL_CHECK", order3=repr(order3), targets=repr(sorted(TARGET_ORDERS)), accepted=(order3 in TARGET_ORDERS))
