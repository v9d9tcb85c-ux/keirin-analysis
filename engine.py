...
ENGINE_VERSION = "2.6-auto-browser-stage-diagnostic-lgrade"
MAX_RETRIES = 3
...
def parse_grade(body):
    """F1/F2/L級を判定。L1/L2/「L級」表記もLとして扱う。"""
    m = re.search(r"(?<![A-Z0-9])(F1|F2|L1|L2)(?![A-Z0-9])", body)
    if m:
        v = m.group(1)
        return "L" if v.startswith("L") else v
    if "L級" in body or "ガールズ" in body or "女子" in body:
        return "L"
    return ""
...
def ensure_playwright_browser():
    """
    Render対策:
    Playwright本体だけ更新され Chromium 実体が消えている場合、
    検索開始前に自動で chromium をインストールする。
    DashboardのBuild Command変更を必須にしない。
    """
    ...
    with _BROWSER_LOCK:
        ...
        try:
            ...
            if exe and Path(exe).exists():
                _log(f"BROWSER_OK path={exe}")
                _BROWSER_READY = True
                return True
        except Exception as e:
            _log(f"BROWSER_CHECK_FAIL {type(e).__name__}: {e}")

        _log("BROWSER_INSTALL_START chromium")
        ...
        try:
            ...
            _log(f"BROWSER_INSTALL_EXIT code={proc.returncode}")
            if tail:
                _log("BROWSER_INSTALL_LOG " + tail.replace("\n", " | "))
            ...
            try:
                ...
                from pathlib import Path
                if exe and Path(exe).exists():
                    _log(f"BROWSER_INSTALL_OK path={exe}")
                    _BROWSER_READY = True
                    return True
            except Exception as e:
                _log(f"BROWSER_RECHECK_FAIL {type(e).__name__}: {e}")

        except Exception as e:
            _log(f"BROWSER_INSTALL_FAIL {type(e).__name__}: {e}")
...
def _process_one_venue(playwright, venue, slug, today, progress=None, venue_index=0, venue_total=0):
    ...
    try:
        ...
        try:
            ...
            errors.append(msg)
            return matches, unpublished, errors, checked, stats
        ...
        for ri, (result_url, sid, day_no, race_no) in enumerate(candidates, 1):
            ...
            try:
                ...
                if grade == "L":
                    stats["girls_l"] += 1
                    _race_diag(venue, race_no, grade="L", result="SKIP_GIRLS_L")
                    _log(f"SKIP_GIRLS_L venue={venue} race={race_no}R")
                    # L級=ガールズ。対象検索はF2なので通常ライン判定には入れない。
                    continue

                if grade != "F2":
                    stats["non_f2"] += 1
                    _race_diag(venue, race_no, grade=grade, result="SKIP_NON_F2")
                    _log(f"SKIP_NON_F2 venue={venue} race={race_no}R grade={grade}")
                    # 重要: breakではなくcontinue。
                    # その競輪場の残りレースも必ず確認する。
                    continue
...
def scan_today(progress=None):
    ...
    if not ensure_playwright_browser():
        raise RuntimeError(
            "Chromiumの自動準備に失敗しました。Renderログの "
            "BROWSER_INSTALL_LOG を確認してください。"
        )
    ...
    checked_total = 0
    stage_stats = {
        "checked": 0, "f2": 0, "girls_l": 0, "star2": 0,
        "marks_ok": 0, "line_target": 0, "order_target": 0, "matched": 0,
        "grade_wait": 0, "star_wait": 0, "marks_wait": 0,
        "line_wait": 0, "order_wait": 0, "non_f2": 0,
        "other_line": 0, "other_order": 0,
    }
    ...
    with sync_playwright() as p:
        ...
        for vi, (venue, slug) in enumerate(venue_items, 1):
            ...
            try:
                matches, unpublished, errors, checked, venue_stats = _process_one_venue(
                    p,
                    venue,
                    slug,
                    today,
                ...
                for k in stage_stats:
                    stage_stats[k] += int(venue_stats.get(k, 0) or 0)

            except Exception as e:
                _log(f"VENUE_EXCEPTION venue={venue} error={type(e).__name__}: {e}")
    ...
    _log(
        "SCAN_DONE "
        f"checked={checked_total} f2={stage_stats['f2']} girlsL={stage_stats['girls_l']} "
        f"star2={stage_stats['star2']} lineTarget={stage_stats['line_target']} "
        f"orderTarget={stage_stats['order_target']} matches={len(all_matches)} "
        f"unpublished={len(all_unpublished)} errors={len(all_errors)}"
    )
    return {
