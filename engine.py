    ...
    try:
        ...
        for ri, (result_url, sid, day_no, race_no) in enumerate(candidates, 1):
            ...
            try:
                ...
                # 固定条件: WINTICKET AI予想「星2」だけを対象にする。
                # 星数そのものがまだ取得できない場合は「非該当」ではなく再判定待ち。
                if conf is None:
                    unpublished.append({
                        "venue": venue,
                        "race": race_no,
                        "reason": "AI予想の星数未公開・未取得",
                        "prediction_url": pred_url
                    })
                    _log(f"STAR_WAIT venue={venue} race={race_no}R")
                    continue

                if conf != TARGET_STAR:
                    _log(f"SKIP_STAR venue={venue} race={race_no}R star={conf}")
                    continue

                _log(f"STAR2 venue={venue} race={race_no}R")
