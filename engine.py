...
def scan_today(progress=None):
    """今日の全場をWINTICKETで確認し、固定条件に合うレースだけ返す。
    AI予想/ラインがまだ出ていないレースはエラーではなく未公開扱い。
    """
    today = date.today()
    matches = []
    errors = []
    ...
    with sync_playwright() as p:
        ...
        for vi, (venue, slug) in enumerate(VENUES.items(), 1):
            if progress:
                progress({"phase":"venues","current":venue,"done":vi-1,"total":total_venues})
            ...
            try:
                ...
                for i in range(links.count()):
                    ...
                    key = (venue, race_no, full)
                    if key not in seen:
                        seen.add(key)
                        candidates.append((venue, slug, full, sid, day_no, race_no))
            except Exception as e:
                errors.append(f"{venue}: {e}")

        total = len(candidates)
        for idx, (venue, slug, result_url, sid, day_no, race_no) in enumerate(candidates, 1):
            if progress:
                progress({
                    "phase":"races","current":f"{venue} {race_no}R",
            ...
            try:
                ...
                # 404/ページ未生成など、予想ページそのものがまだ無いケース
                unpublished.append({
                    "venue": venue, "race": race_no, "reason": "AI予想ページ未公開",
            ...
            try:
                ...
                # グレード自体がまだ取れない場合も未公開扱い
                if not grade:
                    unpublished.append({
                        "venue": venue, "race": race_no, "reason": "レース情報未公開",
                        "prediction_url": pred_url
                    })
                    continue
                if grade != "F2":
                ...
                if hon in ("", None) or tai in ("", None):
                    unpublished.append({
                        "venue": venue, "race": race_no, "reason": "AI予想未公開",
                        "prediction_url": pred_url
                    })
                    continue
                ...
                # ラインが空なら未公開。ガールズ等の特殊形はここでは対象条件外。
                if not line or not groups:
                    unpublished.append({
                        "venue": venue, "race": race_no, "reason": "ライン未公開",
                        "prediction_url": pred_url
                    })
                    continue
                ...
                # 指定ラインなのに3人ラインの印が作れない = AI/ラインどちらか未公開
                if not order3 or "他" in order3 and hon in ("", None):
                    unpublished.append({
                        "venue": venue, "race": race_no, "reason": "並び判定待ち",
                        "prediction_url": pred_url
                    })
                    continue
                ...
                # 時刻などは結果側ページを利用。取れなくても該当判定自体は残す。
                start_time = ""
                session = ""
                try:
                ...
                # ページが存在し、構造異常など本当に想定外ならエラー
                if _looks_unpublished(pred_body):
                    unpublished.append({
                        "venue": venue, "race": race_no, "reason": "予想・ライン未公開",
                        "prediction_url": pred_url
                    })
                else:
                    errors.append(f"{venue} {race_no}R: {e}")

        browser.close()

    def time_key(x):
        t = x.get("time") or "99:99"
        return (t, x.get("venue",""), x.get("race",0))
    matches.sort(key=time_key)
    unpublished.sort(key=lambda x:(x.get("venue",""), x.get("race",0)))

    if progress:
        progress({"phase":"done","current":"完了","done":total,"total":total})
